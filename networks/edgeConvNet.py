...
def edgeFuncAlt(num_points, points, features):
    with tf.name_scope('edgeFunc'):
        ...
        after_gather = tf.gather_nd(features, newPositions) # features shape = (2,11,2) newPositions shape = (2*11*3, 1)
...
def _particle_net_base(points, features=None, mask=None, setting=None, name='particle_net'):
    ...
    with tf.name_scope(name):
        ...
        if mask is not None:
            ...
            coord_shift = tf.multiply(199., tf.cast(tf.equal(mask, 0), dtype='float32'))  # make non-valid positions to 200
...
def loading_data(dataset_name, name_expansion):
    ...
    loaded_arr = loadtxt(home_folder +'data_csv/' + dataset_name +'_points_' + name_expansion + '.csv', delimiter=',')
    points = loaded_arr.reshape((loaded_arr.shape[0],200,3))
    print(f"Points shape: {points.shape}")
    loaded_arr = loadtxt(home_folder + 'data_csv/' + dataset_name + '_features_' + name_expansion + '.csv', delimiter=',')
    ...
    loaded_arr = loadtxt(home_folder + 'data_csv/' + dataset_name + '_labels_' + name_expansion + '.csv', delimiter=',')
...
if __name__ == "__main__":
        ...
        model.fit([points,features,mask], y_data, 
                    epochs = 50,
                    shuffle = True,
                    batch_size = 512,
                    validation_data = ([points_val, features_val, mask_val], y_data_val),
                    callbacks=[reduce_lr, keras.callbacks.EarlyStopping(patience = 10), keras.callbacks.CSVLogger(home_folder + 'edgeConvNetResults/historyParticleNet200_'+ name_of_model +'_.csv')]
                    )
